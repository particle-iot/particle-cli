...
module.exports.run = (args = [], options = {}) => {
	...
	return execa('openssl', [...args], opts);
};
...
module.exports.ensureExists = async () => {
	...
	if (!await exists()){
		throw new OpenSSLError('Unable to locate `openssl` - please ensure it is installed and working');
	}
};
...
class OpenSSLError extends Error {
	constructor(message){
		super(message);
		Error.captureStackTrace(this, OpenSSLError);
	}
}
