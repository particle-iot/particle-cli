...
ApiClient.prototype = {
	...
	flashDevice: function (deviceId, files, targetVersion) {
		...
		var that = this;
		var dfd = when.defer();
		var r = this.request.put('/v1/devices/' + deviceId + '?access_token=' + this._access_token, {
			json: true
		}, function (error, response, body) {
			if (error) {
				return dfd.reject(error);
	...
	compileCode: function(files, platform_id, targetVersion) {
		...
		var that = this;
		var dfd = when.defer();
		var r = this.request.post('/v1/binaries?access_token=' + this._access_token, {
			json: true
		}, function (error, response, body) {
			if (error) {
				return dfd.reject(error);
	...
	downloadBinary: function (url, filename) {
		...
		var that = this;
		var dfd = when.defer();
		console.log('grabbing binary from: ' + url);
		this.request.get(url + '?access_token=' + this._access_token, null,
			function (error, response, body) {
				if (error) {
					return dfd.reject(error);
				}
