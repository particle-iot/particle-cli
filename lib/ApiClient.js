...
ApiClient.prototype = {
	...
	removeCore: function (deviceID) {
		...
		request({
			...
			if (body && body.ok) {
				//console.log("Successfully removed core " + deviceID);
				dfd.resolve(body);
			}
	...
	renameCore: function (deviceID, name) {
		var that = this;
		var dfd = when.defer();
	...
	signalCore: function (deviceID, beSignalling) {
		var dfd = when.defer();
		request({
			uri: this.baseUrl + "/v1/devices/" + deviceID,
			method: "PUT",
	...
	//PUT /v1/devices/{DEVICE_ID}
	flashCore: function (deviceID, files) {
	...
	sendPublicKey: function (deviceID, buffer) {
