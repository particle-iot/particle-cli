...
class BinaryCommand {
	...
	async createProtectedBinary({ file, saveTo, verbose=true }) {
		await this._checkFile(file);

		const resBinaryName = saveTo 
			? (saveTo.endsWith('.bin') ? saveTo : `${saveTo}.bin`)
			: path.basename(file).replace('.bin', '-protected.bin');
