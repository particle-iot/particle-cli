...
describe('Subscribe Command-Line Interface', () => {
	...
	describe('Top-Level `subscribe` Namespace', () => {
		...
		it('Parses options', () => {
			const args = ['subscribe', 'test', '--device', 'my-device', '--until', 'my-data', '--max', '3'];
		...
		it('Parses `--org`', () => {
			const argv = commandProcessor.parse(root, ['subscribe', 'test', '--org', 'my-org']);
			expect(argv.clierror).to.equal(undefined);
			expect(argv.params).to.eql({ event: 'test' });
			expect(argv.org).to.equal('my-org');
		});

		it('Rejects the removed `--all` flag as an unknown argument', () => {
			const argv = commandProcessor.parse(root, ['subscribe', 'test', '--all']);
			expect(argv.clierror).to.be.an.instanceof(Error);
			expect(argv.clierror.message).to.include('all');
		});
